...
RSpec.describe Lsql::SqlExecutor do
  let(:options) { double('options', no_color: false, space: nil, env: 'dev', cluster: nil, database: nil) }
  ...
  describe '#execute_interactive' do
    context 'with no-color option' do
      let(:options) { double('options', no_color: true, space: 'prod', env: 'production', cluster: nil, database: nil) }
      ...
      context 'with dev space' do
        let(:options) { double('options', no_color: true, space: 'dev', env: 'development', cluster: nil, database: nil) }
        let(:database_connector) { double('database_connector', mode_display: '', extract_hostname: 'test-host') }
