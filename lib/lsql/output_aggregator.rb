    ...
    def aggregate_output(output_file = nil)
      ...
      begin
        @temp_files.each_with_index do |temp_info, index|
          ...
          next unless File.exist?(temp_file.path) && File.size(temp_file.path) > 0
          
          lines = File.readlines(temp_file.path)
          next if lines.empty?
          
          if index == 0 || !@header_written
            ...
            if header_lines.any?
              ...
              header_lines.each_with_index do |line, line_index|
                ...
                elsif line_index == 0 && !line.strip.empty?
    ...
    def extract_header_lines(lines)
      ...
      lines.each_with_index do |line, index|
        ...
        if line.match?(/^\s*-+(\+.*)?-*\s*$/) || line.match?(/^\s*=+\s*$/)
          ...
          header_lines << line
          break
        elsif index == 0 || line.strip.empty? || is_likely_header(line, index)
          header_lines << line
        else
